...
**Tip** Unless you need the [nss_wrapper][nsswrapper], you can also use the **second generation** (G2) image [accetto/xubuntu-vnc-novnc][accetto-docker-xubuntu-vnc-novnc], which is a streamlined version of this image ([image hierarchy][accetto-xubuntu-vnc-novnc-wiki-image-hierarchy]). The image supports also the **sudo** command.
...
**This image** `accetto/ubuntu-vnc-xfce` is the base image of my other Docker images with additional features ([accetto/ubuntu-vnc-xfce-firefox-plus][accetto-docker-ubuntu-vnc-xfce-firefox-plus], [accetto/ubuntu-vnc-xfce-firefox-default][accetto-docker-ubuntu-vnc-xfce-firefox-default] and [accetto/ubuntu-vnc-xfce-chromium][accetto-docker-ubuntu-vnc-xfce-chromium]).
...
While the containers do not include any web browsers, the [mousepad][mousepad] and [vim][vim] editors and the **ping** utility are already included. Other applications can be easily added also at runtime, because the containers run under the privileged **root** user.
...
This project has been originally inspired by the old docker image [consol/ubuntu-xfce-vnc][consol-docker-ubuntu-xfce-vnc] and derived from the old first version of the GitHub repository [ConSol/docker-headless-vnc-container][consol-github-docker-headless-vnc-container].
